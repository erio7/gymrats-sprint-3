...
export default function App() {
  ...
  return <div className="h-screen bg-[#F7F7FB] text-[#17131F] font-sans flex flex-col relative overflow-hidden">
    ...
    <header className="relative z-10 border-b border-[#E9E5F0] bg-white/90 backdrop-blur-md shadow-sm"><div className="w-full max-w-[1600px] mx-auto px-2 sm:px-6 lg:px-12 py-2.5 sm:py-3 flex flex-col sm:flex-row sm:flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><img src={TD_LOGO_URL} alt="TD Business" className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl object-cover shadow-sm" /><div><h1 className="text-base sm:text-xl font-black text-[#17131F] tracking-tight uppercase">GYM RATS <span className="text-brand">SPRINT 3</span></h1><p className="text-[10px] sm:text-[11px] text-[#7B7286] font-medium uppercase tracking-wider flex items-center gap-1.5 flex-wrap mt-0.5"><Users className="w-3 h-3 text-brand" /> {totalMembers} Competidores{lastUpdate && <><span className="opacity-30">·</span><Clock className="w-3 h-3" /> {lastUpdate}</>}</p></div></div><div className="flex items-stretch justify-center gap-2 sm:gap-3 pb-0.5 w-full sm:w-auto"><div className="hidden md:flex items-stretch gap-2"><StatCard icon={Route} color="text-[#00A97A]" value={totalKm} label="Km Percorridos" /><StatCard icon={Trophy} color="text-brand" value={totalPoints} label="Total de Pontos" /></div><ChallengeCountdown startDate={CHALLENGE_START} endDate={CHALLENGE_END} /></div></div></header>
    <MediaFeed feedData={feedData} />
    <main className="relative z-10 flex-1 min-h-0 overflow-y-auto w-full flex justify-center"><div className="w-full max-w-[1600px] min-w-0 px-2 sm:px-6 lg:px-12 pb-6 flex flex-col gap-4"><nav className="sticky top-0 z-30 shrink-0 min-h-[60px] flex items-center gap-2 overflow-x-auto py-3 bg-[#F7F7FB]/95 backdrop-blur-md border-b border-[#E9E5F0]">{NAV_ITEMS.map(([id, Icon, label]) => <button key={id} onClick={() => setPage(id)} className={`shrink-0 h-9 flex items-center gap-2 px-4 rounded-xl border text-xs font-black uppercase tracking-wider whitespace-nowrap transition-colors ${page === id ? 'bg-brand border-brand text-white shadow-md shadow-brand/20' : 'bg-white border-[#E8E5EF] text-[#746B80] hover:text-brand hover:border-brand-200'}`}><Icon className="w-4 h-4 shrink-0" />{label}</button>)}</nav><div className="min-w-0">{content}</div></div></main>
  </div>;
}
