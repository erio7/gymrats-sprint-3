...
export default function App() {
  ...
  return <div className="h-screen bg-[#030305] text-gray-200 font-sans flex flex-col relative overflow-hidden">
    ...
    <header className="relative z-10 border-b border-white/[0.04] bg-[#0a0a0f]/70 backdrop-blur-sm"><div className="max-w-7xl mx-auto px-4 sm:px-6 py-2.5 sm:py-3 flex flex-col sm:flex-row sm:flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-3"><img src={TD_LOGO_URL} alt="TD Business" className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg object-cover bg-[#12121a]" /><div><h1 className="text-base sm:text-xl font-black text-white tracking-tight uppercase">GYM RATS <span className="text-blue-500">SPRINT 3</span></h1><p className="text-[10px] sm:text-[11px] text-gray-500 font-medium uppercase tracking-wider flex items-center gap-1.5 flex-wrap mt-0.5"><Users className="w-3 h-3" /> {totalMembers} Competidores{lastUpdate && <><span className="opacity-30">·</span><Clock className="w-3 h-3" /> {lastUpdate}</>}</p></div></div><div className="flex items-stretch justify-center gap-2 sm:gap-3 pb-0.5 w-full sm:w-auto"><div className="hidden md:flex items-stretch gap-2"><StatCard icon={Route} color="text-green-500" value={totalKm} label="Km Percorridos" /><StatCard icon={Trophy} color="text-blue-500" value={totalPoints} label="Total de Pontos" /></div><ChallengeCountdown startDate={CHALLENGE_START} endDate={CHALLENGE_END} /></div></div></header>
    <MediaFeed feedData={feedData} />
    <main className="relative z-10 flex-1 min-h-0 overflow-y-auto w-full flex justify-center"><div className="w-full max-w-[1600px] min-w-0 px-2 sm:px-6 lg:px-12 pb-6 flex flex-col gap-4"><nav className="sticky top-0 z-30 shrink-0 min-h-[60px] flex items-center gap-2 overflow-x-auto py-3 bg-[#030305]/95 backdrop-blur-md border-b border-white/[0.04]">{NAV_ITEMS.map(([id, Icon, label]) => <button key={id} onClick={() => setPage(id)} className={`shrink-0 h-9 flex items-center gap-2 px-4 rounded-xl text-xs font-black uppercase tracking-wider whitespace-nowrap transition-colors ${page === id ? 'bg-blue-600 text-white' : 'bg-white/[0.04] text-gray-500 hover:text-gray-300'}`}><Icon className="w-4 h-4 shrink-0" />{label}</button>)}</nav><div className="min-w-0">{content}</div></div></main>
  </div>;
}
