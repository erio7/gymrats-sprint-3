...
export default function App() {
  const { data, feedData, totalKm, loading, error } = useGoogleSheetsData({ rankingUrl: CSV_URL, feedUrl: FEED_CSV_URL, datasetUrl: DATASET_CSV_URL, refreshIntervalMs: REFRESH_INTERVAL_MS });
  ...
  return <div className="h-screen h-[100dvh] bg-[#F7F7FB] text-[#17131F] font-sans flex flex-col relative overflow-hidden">
    ...
    <main className="relative z-10 flex-1 min-h-0 overflow-y-auto overscroll-contain [scrollbar-gutter:stable] w-full flex justify-center"><div className="w-full max-w-[1600px] min-w-0 px-2 sm:px-6 lg:px-12 flex flex-col gap-4"><nav className="relative sm:sticky sm:top-0 z-30 shrink-0 grid grid-cols-1 min-[320px]:grid-cols-2 sm:flex sm:min-h-[60px] items-center gap-2 py-2 sm:py-3 bg-[#F7F7FB]/95 backdrop-blur-md border-b border-[#E9E5F0]">{NAV_ITEMS.map(([id, Icon, label]) => <button key={id} onClick={() => setPage(id)} className={`min-w-0 h-9 flex items-center justify-center gap-1.5 sm:gap-2 px-2 sm:px-4 rounded-xl border text-[9px] sm:text-xs font-black uppercase tracking-wider whitespace-nowrap transition-colors ${page === id ? 'bg-brand border-brand text-white shadow-md shadow-brand/20' : 'bg-white border-[#E8E5EF] text-[#746B80] hover:text-brand hover:border-brand-200'}`}><Icon className="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0" />{label}</button>)}</nav><div className="min-w-0">{content}</div><div aria-hidden="true" className="h-[calc(4rem+env(safe-area-inset-bottom))] shrink-0" /></div></main>
  </div>;
}
