...
export function InsightsPanel({ rankingData, view }) {
  ...
  return <div className="w-full max-w-6xl mx-auto space-y-4">
    ...
    {data.biggestGain?.gain > 0 && <div className="panel p-5"><SectionTitle icon={Flame} title="Virada da semana" subtitle="Maior crescimento comparado à semana anterior" /><div className="mt-3 flex items-baseline gap-2"><span className="text-xl font-black text-white">{data.biggestGain.formattedName}</span><span className="text-emerald-400 font-black">+{data.biggestGain.gain} pontos</span></div></div>}
  </div>;
}
...
function HighlightCard({ highlight }) {
  ...
  return <article className="panel relative min-h-[180px] p-5 overflow-hidden flex flex-col"><div className="absolute inset-x-0 top-0 h-0.5" style={{ background: `linear-gradient(90deg, transparent, ${highlight.color}, transparent)` }} /><div className="flex items-start justify-between gap-3"><div className="flex items-center gap-2" style={{ color: highlight.color }}><Award className="w-4 h-4" /><span className="text-[10px] uppercase tracking-widest font-black">{highlight.label}</span></div><span className="shrink-0 rounded-full px-2.5 py-1 text-[10px] font-black" style={{ color: highlight.color, background: `${highlight.color}14`, border: `1px solid ${highlight.color}30` }}>{highlight.value ? `${highlight.value} pts` : '—'}</span></div>
    {!count ? <div className="flex-1 flex flex-col justify-center"><p className="text-base font-black text-gray-300">Aguardando resultado</p><p className="text-xs text-gray-600 mt-1">Ainda não há pontuação registrada.</p></div> : <div className="flex-1 flex flex-col justify-center mt-4"><div className="flex items-center gap-2 text-gray-500"><Users className="w-3.5 h-3.5" /><span className="text-[10px] uppercase tracking-wider font-bold">{count === 1 ? 'Líder' : `${count} participantes no topo`}</span></div>{count === 1 ? <p className="text-xl font-black text-white mt-2 truncate">{preview[0].formattedName}</p> : <div className="flex items-center gap-2 mt-3 min-w-0">{preview.map(member => <span key={member.memberKey} className="min-w-0 max-w-[30%] truncate rounded-lg bg-white/[0.04] px-2.5 py-1.5 text-xs font-bold text-gray-300">{member.formattedName}</span>)}{count > 3 && <span className="shrink-0 rounded-lg px-2 py-1.5 text-xs font-black" style={{ color: highlight.color, background: `${highlight.color}12` }}>+{count - 3}</span>}</div>}</div>}
  </article>;
}
