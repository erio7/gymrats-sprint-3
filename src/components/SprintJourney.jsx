...
export function SprintJourney({ startDate, endDate }) {
  ...
  return <article className="panel h-full min-h-[280px] px-4 py-4 sm:px-5 sm:py-5 overflow-hidden">
    <div className="flex items-start justify-between gap-4">
      <div className="flex items-start gap-2.5">
        ...
        <div><p className="text-[9px] uppercase tracking-[0.2em] font-black text-brand">Calendário da Sprint</p><h3 className="text-base font-black text-[#17131F] mt-0.5">Rumo ao dia 45</h3></div>
      </div>
    ...
    <div className="mt-3.5">
      ...
      <div className="h-2 rounded-full bg-[#EEEAF2] overflow-hidden"><div className="h-full rounded-full bg-gradient-to-r from-brand to-accent transition-[width] duration-700" style={{ width: `${journey.progress}%` }} /></div>
    </div>

    <div className="mt-4 grid grid-cols-[26px_repeat(7,minmax(0,1fr))] gap-x-1 gap-y-1.5 items-center">
      <span />
      {WEEKDAYS.map(day => <span key={day} className="text-center text-[7px] sm:text-[8px] uppercase font-black text-[#91889B]">{day}</span>)}
      {weeks.map((week, weekIndex) => <JourneyWeek key={weekIndex} week={week} weekIndex={weekIndex} totalDays={journey.totalDays} />)}
    </div>

    <div className="flex items-center justify-between gap-3 mt-4 pt-3 border-t border-[#ECE8F1]">
...
function JourneyWeek({ week, weekIndex, totalDays }) {
  ...
  return <>
    ...
    {cells.map((day, index) => day ? <span
      ...
      className={`relative h-6 sm:h-7 rounded-md flex items-center justify-center text-[8px] sm:text-[9px] font-black border transition-colors ${day.status === 'today' ? 'bg-brand border-brand text-white shadow-md shadow-brand/20' : day.status === 'complete' ? 'bg-brand-50 border-brand-100 text-brand' : 'bg-[#FAF9FC] border-[#EEEAF2] text-[#AAA2B2]'} ${day.sprintDay === totalDays ? 'ring-1 ring-[#00A97A]/40' : ''}`}
    >
      {day.sprintDay}
