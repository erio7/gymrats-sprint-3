...
export function SprintJourney({ startDate, endDate, compact = false }) {
  ...
  return <article className={`panel h-full min-h-[280px] overflow-hidden ${compact ? 'sprint-journey-compact p-3' : 'p-4 min-[1700px]:p-5'}`}>
    <div className="flex items-start justify-between gap-4">
      <div className="flex items-start gap-2.5">
        <span className={`journey-icon ${compact ? 'w-7 h-7' : 'w-8 h-8'} rounded-lg bg-brand-50 text-brand flex items-center justify-center shrink-0`}><CalendarDays className="w-4 h-4" /></span>
        <div><p className="text-[9px] uppercase tracking-[0.2em] font-black text-brand">Calendário da Sprint</p><h3 className={`journey-title ${compact ? 'text-sm' : 'text-base'} font-black text-[#17131F] mt-0.5`}>Rumo ao dia 45</h3></div>
      </div>
    ...
    <div className={`journey-grid ${compact ? 'mt-2.5 gap-y-1' : 'mt-4 gap-y-1.5'} grid grid-cols-[26px_repeat(7,minmax(0,1fr))] gap-x-1 items-center`}>
      <span />
    ...
    <div className={`journey-footer flex items-center justify-between gap-3 border-t border-[#ECE8F1] ${compact ? 'mt-2 pt-2' : 'mt-4 pt-3'}`}>
