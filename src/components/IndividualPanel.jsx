...
export function IndividualPanel({ rankingData }) {
  ...
  return <div className="w-full max-w-6xl mx-auto bg-white border border-[#E8E5EF] rounded-2xl shadow-lg overflow-hidden flex flex-col md:max-h-full">
    ...
    <div className="hidden lg:block overflow-y-auto overflow-x-hidden flex-1 min-h-0 scrollbar-thin"><table className="w-full table-fixed border-collapse"><colgroup><col className="w-10" /><col className="w-[180px]" />{Array.from({ length: 14 }).map((_, index) => <col key={index} />)}<col className="w-20" /></colgroup><thead className="sticky top-0 bg-[#F3F0F6] z-10"><tr className="text-[9px] uppercase tracking-widest text-[#746B80] font-bold border-b border-[#E4DFE9]"><th className="px-1 py-3 text-center">#</th><SortableHeader field="name" label="Nome" sortBy={sortBy} setSort={setSort} />{[1, 2, 3, 4, 5, 6, 7].map(week => <th key={week} className="px-1 py-3 text-center leading-tight">Sem<br />{week}</th>)}<th className="px-1 py-3 text-center leading-tight">D1<br />100k</th><th className="px-1 py-3 text-center leading-tight">D2<br />Conv</th><th className="px-1 py-3 text-center leading-tight">D3<br />Equipe</th><th className="px-1 py-3 text-center leading-tight">D4<br />Mãe</th><th className="px-1 py-3 text-center leading-tight">D5<br />Extra</th><th className="px-1 py-3 text-center leading-tight">Relâm.<br />Pose</th><th className="px-1 py-3 text-center leading-tight">Gin-<br />cana</th><SortableHeader field="points" label="Pontos" sortBy={sortBy} setSort={setSort} className="text-center" /></tr></thead>
