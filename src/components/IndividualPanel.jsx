...
export function IndividualPanel({ rankingData }) {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto bg-white border border-[#E8E5EF] rounded-2xl shadow-lg overflow-hidden flex flex-col md:max-h-full">
      <div className="p-4 border-b border-[#ECE8F1] flex flex-col sm:flex-row sm:items-center gap-3">
        <div className="relative flex-1 max-w-md">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#81778D]" />
          <input type="text" placeholder="Buscar competidor..." value={query} onChange={(event) => setQuery(event.target.value)} className="w-full bg-[#FAF9FC] border border-[#E5E0EA] rounded-lg pl-9 pr-3 py-2 text-sm text-[#241D2D] placeholder-[#9A92A3] focus:outline-none focus:border-brand-300 focus:ring-2 focus:ring-brand-100" />
        </div>
        <select value={scoreRange} onChange={(event) => setScoreRange(event.target.value)} className="bg-[#FAF9FC] border border-[#E5E0EA] rounded-lg px-3 py-2 text-xs text-[#504759] focus:outline-none focus:border-brand-300">
        ...
        <span className="text-[10px] text-[#81778D] uppercase tracking-widest font-bold hidden lg:block">Ranking individual</span>
      </div>

      <div className="overflow-auto flex-1 min-h-0 scrollbar-thin">
        <table className="w-full min-w-[1250px] border-collapse">
          <thead className="sticky top-0 bg-[#F3F0F6] z-10">
            <tr className="text-[9px] uppercase tracking-widest text-[#746B80] font-bold border-b border-[#E4DFE9]">
              <th className="px-4 py-3 text-left w-14 sticky left-0 bg-[#F3F0F6] z-20">#</th>
              <SortableHeader field="name" label="Nome" sortBy={sortBy} setSort={setSort} className="sticky left-14 bg-[#F3F0F6] z-20 min-w-[180px]" />
          ...
          <tbody>
            {members.map((member) => (
              <tr key={member.memberKey} className="border-t border-[#F0EDF3] hover:bg-brand-50/45 even:bg-[#FCFBFD] transition-colors group">
                <td className="px-4 py-2.5 text-[#81778D] font-mono text-xs sticky left-0 bg-white group-hover:bg-[#F7F2FF] transition-colors z-10">{member.rank}</td>
                <td className="px-4 py-2.5 sticky left-14 bg-white group-hover:bg-[#F7F2FF] transition-colors z-10">
                  <MemberTooltip member={member} accentColor={ACCENT}>
                    <div className="flex items-center gap-2 cursor-help">
                      <span className="text-[#241D2D] text-sm font-semibold truncate">{member.formattedName}</span>
                ...
                <td className="px-4 py-2.5 text-center text-brand text-sm font-black">{member.points}</td>
              </tr>
            ))}
          </tbody>
          <tfoot className="sticky bottom-0 bg-[#F3F0F6] z-20 border-t-2 border-[#DED8E4]">
            <tr className="text-[10px] uppercase tracking-wider text-[#746B80] font-bold"><td className="px-4 py-3 sticky left-0 bg-[#F3F0F6] z-30">TOTAL</td><td className="px-4 py-3 sticky left-14 bg-[#F3F0F6] z-30">SOMA EXIBIDA</td><td colSpan="14" className="bg-[#F3F0F6]" /><td className="px-4 py-3 text-center font-mono font-black text-[#008C66] text-sm bg-[#F3F0F6]">{formatTotal(totalPoints)}</td></tr>
          </tfoot>
        </table>
...
const ValueCell = ({ value }) => <td className="px-3 py-2.5 text-center font-mono text-xs"><span className="font-bold" style={{ color: !value || value === '0' || value === '0,0' ? '#A39AAE' : ACCENT }}>{value || '0'}</span></td>;
...
function SortableHeader({ field, label, sortBy, setSort, className = '' }) {
  ...
  return <th className={`px-4 py-3 text-left cursor-pointer select-none hover:text-brand ${className}`} onClick={() => setSort(field)}><div className="flex items-center gap-1.5"><span>{label}</span><Icon className={`w-3 h-3 ${active ? 'text-brand' : 'opacity-40'}`} /></div></th>;
}
