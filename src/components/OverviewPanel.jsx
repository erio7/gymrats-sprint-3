...
export function OverviewPanel({ rankingData }) {
  ...
  return <div className="w-full max-w-6xl mx-auto space-y-4">
    <section className="panel px-4 pt-5 sm:px-8 sm:pt-7 overflow-hidden"><div className="flex items-start justify-between"><div><p className="text-[10px] uppercase tracking-[0.24em] font-black text-blue-400">Ranking geral</p><h2 className="text-xl sm:text-2xl font-black text-white mt-1">Top 5 da Sprint</h2></div><span className="hidden sm:block text-[10px] uppercase tracking-widest text-gray-600 font-bold">Passe o mouse para detalhes</span></div>
      <div className="hidden md:grid grid-cols-5 gap-3 items-end min-h-[370px] mt-6">{DISPLAY_ORDER.map((index, displayIndex) => { const member = topFive[index]; return member ? <PodiumSlot key={member.memberKey} member={member} baseHeight={BASE_HEIGHTS[displayIndex]} /> : <div key={index} />; })}</div>
      <div className="md:hidden space-y-2 mt-5 pb-4">{topFive.map(member => <MobileRow key={member.memberKey} member={member} />)}</div>
    </section>
...
function MobileRow({ member }) { const medal = MEDAL_COLORS[member.rank - 1] || '#3B82F6'; return <MemberTooltip member={member} accentColor={medal}><div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-white/[0.03] cursor-help"><span className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-black" style={{ background: medal, color: member.rank <= 3 ? '#09090b' : '#fff' }}>{member.rank}º</span><span className="flex-1 text-sm font-bold text-white">{member.formattedName}</span><Trend trend={member.trend} /><strong className="text-lg text-white">{member.points}</strong></div></MemberTooltip>; }
