...
function PodiumCard({ member, offset }) {
  ...
  return <div className="min-w-0" style={{ marginTop: offset, flex: flexSize, zIndex: winner ? 2 : 1 }}><MemberTooltip member={member} accentColor={medal}><article className="relative rounded-2xl overflow-hidden border px-4 py-4 flex flex-col cursor-help transition-all duration-200 hover:-translate-y-1" style={{ height: cardHeight, background: winner ? 'linear-gradient(145deg, rgba(246,201,69,.14), rgba(17,17,25,.98) 52%)' : 'linear-gradient(145deg, rgba(91,140,255,.08), rgba(17,17,25,.98) 58%)', borderColor: winner ? 'rgba(246,201,69,.46)' : 'rgba(255,255,255,.09)', boxShadow: winner ? '0 22px 55px rgba(246,201,69,.10), 0 16px 40px rgba(0,0,0,.30)' : '0 14px 34px rgba(0,0,0,.25)' }}>
...
function Progress({ value, color }) { return <div className="h-1.5 w-full rounded-full bg-white/[0.07] mt-2 overflow-hidden"><div className="h-full rounded-full" style={{ width: `${value}%`, background: `linear-gradient(90deg, ${color}90, ${color})` }} /></div>; }
